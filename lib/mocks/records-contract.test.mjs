...
const parcels = [
  ...
  { id: "owner", jurisdictionId: "j-local", registryStatus: "verified", dagNo: "RS-88", khatianNo: "217", title: "Homestead", ownerName: "Karim", ulpin: "ILR-TWO" },
...
test("preview record status and filtering come from related active mutations", () => {
  const active = filterLandOfficeRecords({
    ...
    jurisdictions,
    parcels,
    mutations: [{ parcelId: "owner", status: "under-primary-verification" }],
...
test("preview record status follows flagged, linked-dispute, mutation, plot-dispute, verified precedence", () => {
  const mutations = [{ parcelId: "dag", status: "under-primary-verification" }];
  const disputedMutations = [{ parcelId: "dag", status: "field-investigation", disputeId: "ds-1" }];
  const disputes = [{ parcelId: "dag", status: "under-review" }];
  const documents = [{ parcelId: "dag", verificationStatus: "flagged" }];

  assert.equal(filterLandOfficeRecords({ actor, jurisdictions, parcels, mutations })[0].registryStatus, "under-mutation");
  assert.equal(filterLandOfficeRecords({ actor, jurisdictions, parcels, mutations, disputes })[0].registryStatus, "under-mutation");
  assert.equal(filterLandOfficeRecords({ actor, jurisdictions, parcels, mutations: disputedMutations, disputes })[0].registryStatus, "disputed");
  assert.equal(filterLandOfficeRecords({ actor, jurisdictions, parcels, disputes })[0].registryStatus, "disputed");
  assert.equal(filterLandOfficeRecords({ actor, jurisdictions, parcels, mutations, disputes, documents })[0].registryStatus, "flagged");
  assert.equal(filterLandOfficeRecords({ actor, jurisdictions, parcels })[0].registryStatus, "verified");
  assert.deepEqual(filterLandOfficeRecords({ actor, jurisdictions, parcels, status: "pending" }), []);
});
