...
test("allows only one browser tab to own the synchronization lease", async () => {
  const repo = repository(new IDBFactory(), "sync-lease");
  assert.equal(await repo.acquireSyncLease("tab-a"), true);
  assert.equal(await repo.acquireSyncLease("tab-b"), false);
  await repo.releaseSyncLease("tab-a");
  assert.equal(await repo.acquireSyncLease("tab-b"), true);
});

test("orders lifecycle operations even when UUID order and timestamps cannot", async () => {
  const ids = ["z-start", "a-points"];
  const repo = new FieldOfflineRepository({
    indexedDB: new IDBFactory(),
    dbName: "lifecycle-order",
    now: () => "2026-09-14T05:00:00.000Z",
    createId: () => ids.shift(),
  });
  await repo.createSurvey(survey);
  await repo.appendPoint(point(1));
  assert.deepEqual(
    (await repo.listOperations(survey.key)).map((operation) => operation.operation_type),
    ["START_SURVEY", "APPEND_POINTS"],
  );
});
