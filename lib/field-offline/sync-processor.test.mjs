...
test("completion joins an active GPS upload and drains work queued during that upload", async () => {
  const repo = makeRepo("sync-completion-race");
  await repo.createSurvey(survey);

  let releaseStart;
  const startBlocked = new Promise((resolve) => { releaseStart = resolve; });
  let startSending;
  const startWasSent = new Promise((resolve) => { startSending = resolve; });
  const sent = [];
  let serverVersion = 0;
  const processor = new FieldSyncProcessor(repo, {
    isOnline: () => true,
    send: async (operation) => {
      sent.push(operation.operation_type);
      if (operation.operation_type === "START_SURVEY") {
        startSending();
        await startBlocked;
      }
      serverVersion += 1;
      if (operation.operation_type === "COMPLETE_SURVEY") {
        assert.equal(operation.payload.expectedVersion, 2);
        return { survey: { id: "server-1", version: serverVersion, status: "completed" } };
      }
      return { sessionId: "server-1", version: serverVersion };
    },
  });

  const automaticSync = processor.process(survey.assignedAgentId);
  await startWasSent;
  ...
  await repo.queueCompletion(survey.key, "Survey completed without a dispute.");

  const filingSync = processor.process(survey.assignedAgentId);
  releaseStart();
  await Promise.all([automaticSync, filingSync]);

  assert.deepEqual(sent, ["START_SURVEY", "APPEND_POINTS", "COMPLETE_SURVEY"]);
  assert.deepEqual(
    (await repo.listOperations(survey.key)).map((operation) => operation.sync_status),
    ["SYNCED", "SYNCED", "SYNCED"],
  );
});
