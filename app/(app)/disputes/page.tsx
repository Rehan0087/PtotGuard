...
import { useDisputes, useRole } from "@/hooks/queries";
...
export default function DisputesPage() {
  const t = useT();
  const role = useRole();
  const { data, isLoading } = useDisputes({ scope: role === "citizen" ? "mine" : undefined });
  const disputes = data?.items ?? [];

  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow={role === "citizen" ? t.nav.portals.citizen : t.nav.portals.landOffice}
